...
def perform_calibration(detected_circles):
    ...
    circle0 = detected_circles[0]
    circleN = detected_circles[-1]
...
def start_camera_processing():
    ...
    deflection_plot, = ax_deflection.plot([], [], 'mo-', label='Deflection (ΔY)')
    ...
    while cap.isOpened():
        ...
        # RESOLVED: Removed the entire redundant and error-prone block that was here.
        # The logic is now handled correctly in the following sections.
        ...
        if pixel_scale != 1.0 and locked_positions and len(deflections_mm) >= 3:
            try:
                ...
                if beam_length_L_m > 0 and EI_current_beam > 0:
                    ...
                    shear_N_fit = np.full_like(x_m, -fitted_load_P)
